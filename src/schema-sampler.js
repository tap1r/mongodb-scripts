...
var schema = dbs.map(dbName => ({
    "db": dbName,
    "collections": db.getSiblingDB(dbName).getCollectionInfos({ "type": "collection", "name": { "$ne": "system.views" } }, true, true).map(collection => ({
    ...
    "views": db.getSiblingDB(dbName).getCollectionInfos({ "type": "view" }, true, true).map(view => ({
        "name": view.name,
